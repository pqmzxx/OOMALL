...
public class ProductDao {
    ...
    @Autowired
    public ProductDao(ProductPoMapper productPoMapper, OnSaleDao onSaleDao, ProductAllMapper productAllMapper, @Qualifier("productJPARepository") ProductJPARepository productJPARepository) {
    ...
    public List<Product> findProductByName_JPA(String name) throws BusinessException {
        ...
        ProductPoExample.Criteria criteria = example.createCriteria();
        criteria.andNameEqualTo(name);
        List<ProductPoJPA> productPoJPAList = productJPARepository.findByName(name);
        productPoJPAList.forEach(mainProductlist->{
            Product mainProduct = CloneFactory.copy(new Product(),mainProductlist,false);
            List<Product> otherProducts = productJPARepository.findOtherProductsByGoodsId(
                    mainProductlist.getGoodsId(),mainProductlist.getId()).stream().map(po -> CloneFactory.copy(new Product(),
                    po, false)).collect(Collectors.toList());
